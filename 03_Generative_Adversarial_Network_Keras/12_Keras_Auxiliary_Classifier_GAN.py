...
LOG_DIR_Model = "/tmp/ML/13_Generative_Adversarial_Network/Auxiliary_Classifier_GAN/Model"
LOG_DIR_Graph = "/tmp/ML/13_Generative_Adversarial_Network/Auxiliary_Classifier_GAN/Graph"
...
class ACGAN():
    ...
    def train(self, N_EPISODES, batch_size=128, sample_interval=50):
        ...
        for episode in range(N_EPISODES):
            ...
            if (episode+1) % Print_Cycle == 0:
                print ("[Episodes: %6d] [D loss: %3.6f, acc.: %2.2f%%, op_acc: %2.2f%%] [G loss: %3.6f]" % (episode+1, d_loss[0], 100*d_loss[3], 100*d_loss[4], g_loss[0]))
    ...
    def sample_images(self, episode):
        _row, _column = 10, 10
        noise = np.random.normal(0, 1, (_row * _column, 100))
        sampled_labels = np.array([num for _ in range(_row) for num in range(_column)])
        gen_imgs = self.generator.predict([noise, sampled_labels])
        # Rescale images 0 - 1
...
if __name__ == '__main__':
    ...
    acgan = ACGAN()
    acgan.train(N_EPISODES=5000, batch_size=32, sample_interval=200)
