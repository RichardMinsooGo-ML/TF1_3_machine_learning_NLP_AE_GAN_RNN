...
SAVE_DIR = "/tmp/ML/13_Generative_Adversarial_Network/19_Eb"
...
def DISCRIMINATOR(X):
    ...
    mse_Dis = tf.reduce_mean(tf.reduce_sum((X - X_recon)**2, 1))
    return mse_Dis
...
Train_D = (tf.train.AdamOptimizer(learning_rate=lr)
            .minimize(D_loss, var_list=D_var_list))
Train_G = (tf.train.AdamOptimizer(learning_rate=lr)
...
for episode in range(N_EPISODES):
    ...
    _, D_loss_curr = sess.run([Train_D, D_loss], feed_dict={X: X_mb, z: z_mb})

    _, G_loss_curr = sess.run(
        [Train_G, G_loss], feed_dict={X: X_mb, z: GET_NOISE(mb_size, NOISE_SIZE)}
    )
